  ...
  <div class="container">
    ...
    <div class="micro-posts-container">
      <div
        ...
        v-bind:index="index"
        v-bind:key="microPost._id"
      >
        <div class="created-at">
          {{
            `${microPost.createdAt.getDate()}/${microPost.createdAt.getMonth() +
              1}/${microPost.createdAt.getFullYear()}`
          }}
        </div>
        <p class="text">{{ microPost.text }}</p>
        <p class="author">
          - {{ microPost.author ? microPost.author.name : "Unknown" }}
        </p>
      </div>
    </div>
  </div>
...
import MicroPostService, { Data } from "../store/MicroPostsService";
import auth0Client from "../store/AuthService";

@Component
export default class HelloWorld extends Vue {
  @Prop() private msg!: string;
  private microPosts: Data[] = [];
  private error = "";
  private profile = "";

  async created() {
    try {
      const posts = await MicroPostService.getMicroPosts();
      this.microPosts = posts;
